...
test("rendering and checking Todo component", async () => {
  ...
  render(
    <Todo
      ...
      todo={tempTodo}
      config={config}
      isCompleted={tempTodo.completed}
      onChange={() => {}}
    />
  );
...
test("testing concantinated names", async () => {
  const names = {
    name: {
      title: "ms",
      first: "Aishwarya",
  ...
  expect(getNames(names)).toBe(" ms Aishwarya Karkera");
});

test("test concatinated location", async () => {
  const location = {
    name: "mystreet",
    number: "5",
    postcode: "23"
